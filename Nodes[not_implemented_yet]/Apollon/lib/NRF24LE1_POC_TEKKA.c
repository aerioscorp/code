...
void setRF24_CHANNEL(int channel) {
    RF24_CHANNEL = channel;
}

void setBaseAddress(uint8_t address) {
    base_addr = address;
}
...
void present(uint8_t childSensorId, uint8_t sensorType, const char *description) {
    uint8_t length = strlen(description);
    strncpy(outMsg.data, description, length);
    BuildMessage(GATEWAY_ADDRESS,childSensorId,C_PRESENTATION,sensorType, P_STRING, length,false);
    SendMessage();
}


inline void sendSketchInfo(const char *name, const char *version) {
    uint8_t length = strlen(name);
    mSetLength(outMsg,length);
    strncpy(outMsg.data, name, length);

    BuildMessage(GATEWAY_ADDRESS, 0xFF, C_INTERNAL, I_SKETCH_NAME, P_STRING, length, false);
    SendMessage();

    length = strlen(version);
    mSetLength(outMsg,length);
    strncpy(outMsg.data, version, length);
    BuildMessage(GATEWAY_ADDRESS, 0xFF, C_INTERNAL, I_SKETCH_VERSION, P_STRING, length, false);
    SendMessage();
}
